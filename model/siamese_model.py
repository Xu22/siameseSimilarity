...
class SiameseLSTM(object):
    ...
    """
    # def _get_rnn_unit(self, rnn_unit, n_hidden):
    #     if rnn_unit == 'lstm':
    #         fw_cell = rnn.BasicLSTMCell(n_hidden, forget_bias=1., state_is_tuple=True)
    #         bw_cell = rnn.BasicLSTMCell(n_hidden, forget_bias=1., state_is_tuple=True)
    #     elif rnn_unit == 'gru':
    ...
    def BiRNN(self, x, dropout, scope, hidden_units):
        ...
        with tf.name_scope("fw" + scope), tf.variable_scope("fw" + scope):
            ...
            for _ in range(n_layers):
                fw_cell = tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0, state_is_tuple=True, name='basic_lstm_cell')
        ...
        with tf.name_scope("bw" + scope), tf.variable_scope("bw" + scope):
            stacked_rnn_bw = []
            for _ in range(n_layers):
                bw_cell = tf.nn.rnn_cell.LSTMCell(n_hidden, forget_bias=1.0, state_is_tuple=True, name='basic_lstm_cell')
